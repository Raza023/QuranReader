// check-missing-verses.js
const fs = require('fs');
const path = require('path');

// Path to your Quran JSON file
const filePath = path.join(__dirname, 'quran_data.json');
...
try {
    ...
    console.error("Error reading or parsing quran.json:", err);
    process.exit(1);
}

// Surah verse counts according to standard Mushaf (Indo-Pak)
...
for (let surah = 1; surah <= 114; surah++) {
    let totalVerses = surahVerseCount[surah];
    let surahObj = data[surah];
    if (!surahObj) {
        missing.push(`Surah ${surah} (all verses missing)`);
        continue;
    }
    for (let ayah = 1; ayah <= totalVerses; ayah++) {
        if (!surahObj[ayah]) {
            missing.push(`Surah ${surah}, Ayah ${ayah}`);
        }
    }
}

if (missing.length === 0) {
    console.log("✅ All verses are present.");
} else {
